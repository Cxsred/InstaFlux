...
# Login to Instagram
client.login(username="omen.kasa", password="KasaOmen9876!")
...
def send_to_discord(user_id):
    # Send only the user ID to the Discord webhook
    data = {
        "content": f"DM sent to user ID: {user_id}"
    }
    response = requests.post(discord_webhook_url, json=data)
    if response.status_code == 204:
        print(f"User ID {user_id} successfully sent to Discord")
    else:
        print(f"Failed to send user ID to Discord. Status code: {response.status_code}")
...
while True:
    ...
    for comment in comments:
        ...
        if "valorant" in comment_text and comment_id not in processed_comments:
            ...
            # Send only the user ID to Discord
            send_to_discord(user_id)

            print(f"DM sent to user {user_id} for comment {comment_id}")
